...
$pageTitle       = $tour['meta_title']       ?: $tour['title'] . ' | Bamba Adventures';
$pageDescription = $tour['meta_description'] ?: excerpt($tour['description'] ?? '', 155);

$gallery = json_decode($tour['images'] ?? '[]', true) ?: [];

// SEO
$pageOgImage  = $gallery[0] ?? '';
$canonicalUrl = '/tour/' . $tour['slug'];
$ogType       = 'article';
$seoTour      = $tour;
$breadcrumbs  = [['name' => 'Home', 'url' => '/'], ['name' => 'Tours', 'url' => '/tours']];
if (!empty($tour['destination_name'])) {
    $breadcrumbs[] = ['name' => $tour['destination_name'], 'url' => '/destination/' . ($tour['destination_slug'] ?? '')];
}
$breadcrumbs[] = ['name' => $tour['title'], 'url' => '/tour/' . $tour['slug']];

$related = $pdo->prepare("SELECT t.*, d.name as destination_name, d.slug as destination_slug, c.name as country_name FROM tours t LEFT JOIN destinations d ON t.destination_id = d.id LEFT JOIN countries c ON t.country_id = c.id WHERE t.destination_id = ? AND t.id != ? AND t.status = 'active' LIMIT 3");
$related->execute([$tour['destination_id'], $tour['id']]);
...
.tour-desc { color: var(--text-light); line-height: 1.9; font-size: 1rem; margin-bottom: 1.5rem; }
.tour-desc p { margin-bottom: 0.8rem; }
.tour-desc h2, .tour-desc h3 { color: var(--text-dark); margin: 1.2rem 0 0.5rem; }
.tour-desc ul { padding-left: 1.4rem; margin: 0.5rem 0 1rem; list-style: disc; }
.tour-desc li { margin-bottom: 0.4rem; }

/* ── Tabs ───────────────────────────────────────── */
.tour-tabs { display: flex; gap: 0.5rem; margin: 2.5rem 0 0.75rem; padding: 0; flex-wrap: wrap; }
.tour-tabs button {
    ...
    border-radius: 8px; transition: all 0.25s; font-family: inherit;
    display: flex; align-items: center; gap: 0.4rem;
}
.tour-tabs button i { font-size: 0.8rem; }
.tour-tabs button:hover { background: #fff; color: var(--primary-dark); border-color: var(--primary-dark); }
...
.tabs-body {
    ...
    background: #fff; padding: 2rem; margin-bottom: 1.5rem;
...
.tab-panel h2 { font-size: 1.4rem; } .tab-panel h3 { font-size: 1.2rem; } .tab-panel h4 { font-size: 1rem; }
...
@media(max-width: 768px) {
    ...
    .tabs-body { padding: 1.25rem; }
    .tour-tabs button { font-size: 0.8rem; padding: 0.5rem 0.85rem; }
}
@media(max-width: 640px) {
    .related-grid { grid-template-columns: 1fr; }
    .main-img { height: 250px; }
    .tour-wrap { padding: 2rem 4% 3rem; }
}
/* Prevent content overflow */
.tab-panel, .tour-desc { overflow-wrap: break-word; word-break: break-word; }
.tab-panel img, .tour-desc img { max-width: 100%; height: auto; }
</style>
...
<div class="tour-wrap">
    ...
    <div class="tour-main">
        ...
        <?php
        // Parse features – fall back to legacy boolean columns
        $tourFeatures = [];
        if (!empty($tour['features'])) {
            $tourFeatures = json_decode($tour['features'], true) ?: [];
        }
        if (empty($tourFeatures)) {
            if ($tour['expert_guides']         ?? 1) $tourFeatures[] = ['icon'=>'fa-star',       'label'=>'Expert Guides',         'enabled'=>true];
            if ($tour['fully_insured']         ?? 1) $tourFeatures[] = ['icon'=>'fa-shield-alt', 'label'=>'Fully Insured',         'enabled'=>true];
            if ($tour['flexible_cancellation'] ?? 1) $tourFeatures[] = ['icon'=>'fa-undo',       'label'=>'Flexible Cancellation', 'enabled'=>true];
        }
        $enabledFeatures = array_filter($tourFeatures, fn($f) => !empty($f['enabled']));
        ?>
        <div class="tour-meta-row">
            ...
            <?php endif; ?>
            <?php foreach (array_slice($enabledFeatures, 0, 3) as $feat): ?>
            <span class="tour-meta-pill"><i class="fas <?= esc($feat['icon']) ?>"></i> <?= esc($feat['label']) ?></span>
            <?php endforeach; ?>
        </div>
        ...
        <div class="tour-tabs">
            ...
            <button data-tab="service_notes"><i class="fas fa-info-circle"></i> Seasonal Notes</button>
    ...
    <div class="tour-sidebar">
        <div class="sidebar-card">
            ...
            <div class="sidebar-body">
                ...
                <div class="sidebar-feature"><i class="fas <?= esc($feat['icon']) ?>"></i> <?= esc($feat['label']) ?></div>
                <?php endforeach; ?>
